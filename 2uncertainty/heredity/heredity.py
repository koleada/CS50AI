...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if mom == None and dad == None:
            ...
            mom_gene_prob = get_gene_prob(mom, one_gene, two_genes)
            dad_gene_prob = get_gene_prob(dad, one_gene, two_genes)
            if person_genes == 2:
                gene_prob = mom_gene_prob * dad_gene_prob
            elif person_genes == 1:
                gene_prob = (mom_gene_prob * mom_gene_prob) + (
                    dad_gene_prob * dad_gene_prob
                )
            else:
                gene_prob = 1 - mom_gene_prob * 1 - dad_gene_prob
            trait_prob = PROBS["trait"][person_genes][person_trait]
        total = gene_prob * trait_prob
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        normal_gene_factor = 1 / total_gene_prob
        normal_trait_factor = 1 / total_trait_prob

        probabilities[person]["gene"][0] = (
            probabilities[person]["gene"][0] * normal_gene_factor
        )
        probabilities[person]["gene"][1] = (
            probabilities[person]["gene"][1] * normal_gene_factor
        )
        probabilities[person]["gene"][2] = (
            probabilities[person]["gene"][2] * normal_gene_factor
        )

        probabilities[person]["trait"][True] = (
            probabilities[person]["trait"][True] * normal_trait_factor
        )
        probabilities[person]["trait"][False] = (
            probabilities[person]["trait"][False] * normal_trait_factor
        )
